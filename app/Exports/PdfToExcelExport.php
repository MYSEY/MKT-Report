    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Wrap Transaction Details, align everything to top
                ...
                // Center Date, right-align amount columns
                $sheet->getStyle("A2:A{$highestRow}")
                    ->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("C2:E{$highestRow}")
                    ->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);

                // Auto row height estimated from wrapped text length
                // (PhpSpreadsheet can't truly auto-fit wrapped row height,
                // so we estimate based on characters per line for column B's width)
