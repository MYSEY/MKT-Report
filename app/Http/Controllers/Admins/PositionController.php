...
class PositionController extends Controller
{
    ...
    {
        $this->applyRolePermissions('Position');
    }
    public static function getPositions(){
        $position = DB::connection('mysqlhrconnection')->table('positions')->orderBy('name_english', 'asc')->get();
        return $position;
    }
    /**
    ...
    {
        if (!$this->denyPermission('Position View')) {
            return view('page.access_page');
        }

        if ($request->ajax()) {
            $query = Position::with(["hrPosition"]);
            $recordsTotal = Position::count();
            $search = $request->input('search.value');
            if (!empty($search)) {
                $query->where(function ($q) use ($search) {
                    // ២. ឆែកមើលថាតើអ្នកប្រើប្រាស់វាយពាក្យដែលត្រូវនឹង "type" ដែរឬទេ
                    if (stripos("Network Employee", $search) !== false) {
                        $q->orWhere('type', '1');
                    } 
                    
                    if (stripos("TMG", $search) !== false) {
                        $q->orWhere('type', '2');
                    }
        ...
        return view('configurations.position.index',compact('positions'));
    }
        ...
        try {
            ...
            foreach ($sheetNames as $id) {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $interestIncome = Position::create([
